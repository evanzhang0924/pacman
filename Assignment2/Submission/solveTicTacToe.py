...
class TicTacToeAgent():
    ...
    def getAction(self, gameState, gameRules):
        ...
        agentsAction = gameState.getLegalActions(gameRules)
        nextAction = agentsAction[0]
        nextUtility = float("-inf")
        actionList = []

        for action in agentsAction:
            utilityValue = self.evaluationFunction(gameState.generateSuccessor(action), gameRules)
            if  nextUtility < utilityValue:
                nextUtility = utilityValue
                nextAction = action
        return nextAction
    ...
    def evaluationFunction(self, gameState, gameRules):
        ...
        for board in gameState.boards:
            types = self.isType(gameRules, board)
            stateList.append(types)

        stateList.sort()

        pPositonSet = {("1", "1", "a"), ("1", "b", "b"), ("1", "b", "c"), ("1", "c", "c"), ("1", "1", "c2"), ("a", "ab", "c"), ("a", "ad", "d"), ("1", "d", "d"),
                       ("a", "c", "c2"), ("1", "ad", "ad"), ("a", "a", "a"), ("a", "a", "c2"), ("c2", "c2", "c2"), ("ad", "c", "d"), ("1", "c2", "c2")}

        if tuple(stateList) in pPositonSet:
            return 1
        else:
            return -1
    ...
    def rotatedFunction(self, board):
        rotatedBoard = [board]
        tempBoard = list(board)
        for i in range(3):
            newBoard = list(board)
            newBoard[0] = tempBoard[6]
            newBoard[1] = tempBoard[3]
            newBoard[2] = tempBoard[0]
            newBoard[3] = tempBoard[7]
            newBoard[5] = tempBoard[1]
            newBoard[6] = tempBoard[8]
            newBoard[7] = tempBoard[5]
            newBoard[8] = tempBoard[2]
            tempBoard = list(newBoard)
            rotatedBoard.append(tempBoard)
        return rotatedBoard


    def symmetry(self, board):
        symBoard = []
        newBoard = list(board)
        newBoard[0] = board[2]
        newBoard[2] = board[0]
    ...
    def transform(self, board):
        wholeBoard = self.rotatedFunction(board)
        for types in self.symmetry(board):
            if types not in wholeBoard:
                wholeBoard.append(types)
        return wholeBoard


    def isType(self, gameRules, board):
        theBoard = self.rank(board)
        if gameRules.deadTest(board):
            return "1"

        set1 = set()
        type1 = [[True, False, False, False, False, False, False, False, False],
                 [False, True, False, False, False, False, False, False, False],
                 [True, False, False, False, False, True, False, True, False],
                 [False, True, False, False, True, False, False, True, False]]
        for i in type1:
            wholeBoard = self.transform(i)
            for j in wholeBoard:
                ranked1 = self.rank(j)
                set1.add(ranked1)
        if theBoard in set1:
        ...
        setA = set()
        typeA = [[True, False, False, False, False, False, False, False, True],
        ...
        setB = set()
        typeB = [[True, False, True, False, False, False, False, False, False],
        ...
        setC = set()
        typeC = [[False, False, False, False, False, False, False, False, False]]
        for i in typeC:
            wholeBoard = self.transform(i)
            for j in wholeBoard:
                rankedC = self.rank(j)
        ...
        setD = set()
        typeD = [[True, True, False, False, False, True, False, False, False],
        ...
        setAB = set()
        typeAB = [[True, True, False, False, True, False, False, False, False],
        ...
        for i in typeAB:
            wholeBoard = self.transform(i)
            for j in wholeBoard:
                rankedAB = self.rank(j)
                setAB.add(rankedAB)
        if theBoard in setAB:
            return "ab"

        setAD = set()
        typeAD = [[True, True, False, False, False, False, False, False, False]]
        for i in typeAD:
            wholeBoard = self.transform(i)
            for j in wholeBoard:
                rankedAD = self.rank(j)
                setAD.add(rankedAD)
        if theBoard in setAD:
            return "ad"

        setC2 = set()
        typeC2 = [[False, False, False, False, True, False, False, False, False]]
        for i in typeC2:
            wholeBoard = self.transform(i)
            for j in wholeBoard:
                rankedC2 = self.rank(j)
                setC2.add(rankedC2)
        if theBoard in setC2:
            return "c2"
